...
def signing_up():
    ...
    with eng.connect() as con:
        ...
        if rows:
            ...
            query = text("INSERT INTO user (name,email,password) VALUES (:name, :email, :password);")
    ...
    if 'user_id' not in session:
        ...
        with eng.connect() as con:
            ...
            if rows:
                ...
                query = text("INSERT INTO review(gid,gname,uid,uname,message) values (:gid,:gname,:uid,:uname,:review);")
                con.execute(query, **data)
